...
import com.irontower.gcbuser.di.component.DaggerUserCarTimeComponent;
...
import com.irontower.gcbuser.mvp.presenter.UserCarTimePresenter;
...
public class UserCarTimeFragment extends BaseFragment<UserCarTimePresenter> implements
    UserCarTimeContract.View, SwipeRefreshLayout.OnRefreshListener {
  ...
  @Inject
  UseCarTimeAdapter adapter;

  public static UserCarTimeFragment newInstance() {
    UserCarTimeFragment fragment = new UserCarTimeFragment();
    return fragment;
  }

  @Override
  public void setupFragmentComponent(AppComponent appComponent) {
    DaggerUserCarTimeComponent //如找不到该类,请编译一下项目
        .builder()
        .appComponent(appComponent)
        .userCarTimeModule(new UserCarTimeModule(this))
        .build()
        .inject(this);
  }
  ...
  public View initView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    view = inflater.inflate(R.layout.fragment_user_car_time, container, false);
    return view;
  }
  ...
  public void onResume() {
    ...
    MobclickAgent.onPageStart(
        "UserCarTimeFragment"); //统计页面(仅有Activity的应用中SDK自动调用，不需要单独写。"SplashScreen"为页面名称，可自定义)
  }
  ...
  public void onPause() {
    ...
    MobclickAgent.onPageEnd(
        "UserCarTimeFragment"); // （仅有Activity的应用中SDK自动调用，不需要单独写）保证 onPageEnd 在onPause 之前调用,因为 onPause 中会保存信息。"SplashScreen"为页面名称，可自定义
  }
  ...
  public void initChartData(List<ChartData> shortData, List<ChartData> longData) {
    ...
    if (lineChart.getData() != null &&
      ...
      set1 = new LineDataSet(shortEntry, "市区用车数");
...
//      set1.setDrawValues(false);
      ...
      set2 = new LineDataSet(longEntry, "长途用车数");
