...
@ActivityScope
public class ApproveDetailPresenter extends
    BasePresenter<ApproveDetailContract.Model, ApproveDetailContract.View> {
  ...
  @Inject
  public ApproveDetailPresenter(ApproveDetailContract.Model model,
      ApproveDetailContract.View rootView
  ...
  public void approveOrder(String todoId, String s, String s1) {

    mModel.approveOrder(todoId, s, s1).subscribeOn(Schedulers.io())
        .doOnSubscribe(disposable -> mRootView.showLoading())
        .observeOn(AndroidSchedulers.mainThread()).doFinally(() -> {
      mRootView.hideLoading();
    }).compose(RxLifecycleUtils.bindToLifecycle(mRootView))
        .subscribe(new ErrorHandleSubscriber<BaseJson<Map<String, Object>>>(mErrorHandler) {
          ...
          public void onNext(BaseJson<Map<String, Object>> stringBaseJson) {
            if (stringBaseJson.isSuccess()) {
              ...
              ArmsUtils.makeText(mRootView.getActivity(), "审批订单成功");
              EventBus.getDefault().post("1", EventBusTags.APPROVE_ORDER);
