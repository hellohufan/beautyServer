...
public class OrderRecordDetailPresenter extends
  ...
  public void cancelOrder(String orderCarNo, String remark) {
//    mModel.cancelOrderCar(orderCarNo,remark).subscribeOn(Schedulers.io())
//        .doOnSubscribe(disposable -> mRootView.showLoading())
//        .observeOn(AndroidSchedulers.mainThread()).doFinally(() -> {
//      mRootView.hideLoading();
//    }).compose(RxLifecycleUtils.bindToLifecycle(mRootView))
...
//              EventBus.getDefault().post("1", EventBusTags.WITH_DRAW_ORDER);
...
//        });
  ...
  public void withDrawOrder(String orderNo) {
    mModel.withDrawOrder(orderNo).subscribeOn(Schedulers.io())
        .doOnSubscribe(disposable -> mRootView.showLoading())
        .observeOn(AndroidSchedulers.mainThread()).doFinally(() -> {
      mRootView.hideLoading();
    }).compose(RxLifecycleUtils.bindToLifecycle(mRootView))
        .subscribe(new ErrorHandleSubscriber<BaseJson<Map<String, Object>>>(mErrorHandler) {
          ...
          public void onNext(BaseJson<Map<String, Object>> stringBaseJson) {
            if (stringBaseJson.isSuccess()) {
              ArmsUtils.makeText(mRootView.getActivity(), "撤销订单成功");
              EventBus.getDefault().post("1", EventBusTags.WITH_DRAW_ORDER);
              mRootView.killMyself();
        ...
        .subscribe(new ErrorHandleSubscriber<BaseJson<Object>>(mErrorHandler) {
          ...
          public void onNext(BaseJson<Object> stringBaseJson) {
            ...
            if (stringBaseJson.isSuccess()) {
              ...
              ArmsUtils.makeText(mRootView.getActivity(), "删除订单成功");
