...
public class JiGuangReceiver extends BroadcastReceiver {
  private static final String PUSH_CHANNEL_ID = "hasimtech_gcb_user_id";
  private static final String PUSH_CHANNEL_NAME = "hasimtech_gcb_user_name";
          ...
          if (!StringUtils.isEmpty(bundle.getString(JPushInterface.EXTRA_EXTRA))) {
            if (SPUtils.getInstance().getBoolean("isLogin")) {
              ...
              NotificationCompat.Builder builder = new NotificationCompat.Builder(context,
                  ...
                  .setContentTitle("铁塔车管家用户端").setSmallIcon(R.mipmap.ic_launcher)
                  .setTicker("新消息")
                  .setAutoCancel(true)
              ...
              if (map.containsKey("page")) {
                switch (map.get("page")) {
                  case "10000":
                    ...
                    jumpIntent.putExtra("orderNo", map.get("order"));
                    jumpIntent.setClass(context, OrderRecordDetailActivity.class);
                    break;
                  case "10001":
                    jumpIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    jumpIntent.putExtra("todoId", map.get("order"));
                    jumpIntent.setClass(context, ApproveDetailActivity.class);
                    break;
                  case "10002":
                    jumpIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    jumpIntent.putExtra("todoId", map.get("order"));
                    jumpIntent.setClass(context, DispatchVehicleDetailActivity.class);
                    break;
                  case "10004":
                    jumpIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    ...
                    if (!am.isMusicActive()) {
                      if (SPUtils.getInstance().getBoolean("sound")) {
                        switch (map.get("sound")) {
                          case "userCar.wav":
                            MediaPlayer mp = MediaPlayer.create(context, R.raw.usecar);//重新设置要播放的音频
                            mp.start();//开始播放
                            break;
                          case "waiting.wav":
                            MediaPlayer mp2 = MediaPlayer
                                .create(context, R.raw.waiting);//重新设置要播放的音频
                            mp2.start();//开始播放
                            break;
