...
import com.irontower.gcbuser.di.component.DaggerGoingOrderComponent;
...
import com.irontower.gcbuser.mvp.presenter.GoingOrderPresenter;
...
public class GoingOrderFragment extends BaseFragment<GoingOrderPresenter> implements
    GoingOrderContract.View, SwipeRefreshLayout.OnRefreshListener {
  ...
  @Inject
  GoingOrderAdapter adapter;

  private Context context;

  public static GoingOrderFragment newInstance() {
    GoingOrderFragment fragment = new GoingOrderFragment();
    return fragment;
  }

  @Override
  public void setupFragmentComponent(AppComponent appComponent) {
    DaggerGoingOrderComponent //如找不到该类,请编译一下项目
        .builder()
        .appComponent(appComponent)
        .goingOrderModule(new GoingOrderModule(this))
        .build()
        .inject(this);
  }
  ...
  public View initView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    View view = inflater.inflate(R.layout.fragment_going_order, container, false);
...
//
  ...
  @Subscriber(tag = EventBusTags.WITH_DRAW_ORDER, mode = ThreadMode.MAIN)
  ...
  public void onResume() {
    ...
    MobclickAgent.onPageStart(
        "GoingOrderFragment"); //统计页面(仅有Activity的应用中SDK自动调用，不需要单独写。"SplashScreen"为页面名称，可自定义)
  }
  ...
  public void onPause() {
    ...
    MobclickAgent.onPageEnd(
        "GoingOrderFragment"); // （仅有Activity的应用中SDK自动调用，不需要单独写）保证 onPageEnd 在onPause 之前调用,因为 onPause 中会保存信息。"SplashScreen"为页面名称，可自定义
  }
