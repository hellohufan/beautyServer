...
@ActivityScope
public class ApproveGoOutModel extends BaseModel implements ApproveGoOutContract.Model {

  private Gson mGson;
  private Application mApplication;

  @Inject
  public ApproveGoOutModel(IRepositoryManager repositoryManager, Gson gson,
  ...
  @Override
  public Observable<BaseJson<Object>> approvalBegGoOut(String hid, String operDesc, String state) {
    FormBody body = new FormBody.Builder()
        .add("hid", hid)

        .add("operDesc", operDesc)

        .add("state", state)

        .build();

    return mRepositoryManager.obtainRetrofitService(CommonService.class).approvalBegGoOut(
        SPUtils.getInstance().getString(
            CommonService.ACCESS_TOKEN), body);
  }
}