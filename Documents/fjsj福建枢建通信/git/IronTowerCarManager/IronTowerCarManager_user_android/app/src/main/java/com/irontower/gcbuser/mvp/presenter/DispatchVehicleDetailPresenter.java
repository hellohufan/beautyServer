...
@ActivityScope
public class DispatchVehicleDetailPresenter extends
    BasePresenter<DispatchVehicleDetailContract.Model, DispatchVehicleDetailContract.View> {

  private RxErrorHandler mErrorHandler;
  private Application mApplication;

  private TimeLineAdapter mTimeLineAdapter;
  private OrderVehicleAdapter mOrderVehicleAdapter;
  private Gson mGson;

  @Inject
  public DispatchVehicleDetailPresenter(DispatchVehicleDetailContract.Model model,
      DispatchVehicleDetailContract.View rootView
      , RxErrorHandler handler, Application application
      , TimeLineAdapter timeLineAdapter, OrderVehicleAdapter orderVehicleAdapter, Gson gson) {
    super(model, rootView);
    this.mErrorHandler = handler;
    this.mApplication = application;

    this.mTimeLineAdapter = timeLineAdapter;
    this.mOrderVehicleAdapter = orderVehicleAdapter;
    this.mGson = gson;
  }
  ...
  public void finishDispatchCar(String todoId) {
    mModel.finishDispatchCar(todoId).subscribeOn(Schedulers.io())
        .doOnSubscribe(disposable -> mRootView.showLoading())
        .observeOn(AndroidSchedulers.mainThread()).doFinally(() -> {
      mRootView.hideLoading();
    }).compose(RxLifecycleUtils.bindToLifecycle(mRootView))
        .subscribe(new ErrorHandleSubscriber<BaseJson<Object>>(mErrorHandler) {
          ...
          public void onNext(BaseJson<Object> stringBaseJson) {
            if (stringBaseJson.isSuccess()) {
              ArmsUtils.makeText(mRootView.getActivity(), "结束派车成功");
              EventBus.getDefault().post("1", EventBusTags.FINISH_DISPATCH_CAR);
              mRootView.killMyself();
            } else {
